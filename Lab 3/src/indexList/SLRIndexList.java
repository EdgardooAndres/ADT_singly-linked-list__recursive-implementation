...
public class SLRIndexList<E> implements IndexList<E> {
	...
	{ 
		...
		SNode<E> node = new SNode<>(etr) ;
		recRemove(node,index);
	...
	/**Recursively removes the elements in descending order starting
	 * from index 'i' until the first element.
	 * 
	 * @param the node to remove.
	 * @param begining index. has to be the highest to remove.
	 * @return each node that has been removed.
	 **/
	...
	{ 
		// adds a new node (containing e) as the i'th node in
	...
	/**Recursively sets the specified element to the list starting from index 'index'
	 * until the first element in the list.
	 * 
	 * @param f
	 * @param index
	 * @param e
	...
	{ 
		...
		if (index==0) 
		{
			...
			f.setElement(recSet(f.getNext(), index-1, e)); 
			return e; 
		}		
		//return null;   // just to be able to compile
	} 
